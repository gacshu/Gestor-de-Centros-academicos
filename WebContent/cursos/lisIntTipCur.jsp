...
<%
    Vector       listAluTipCur = new Vector();
    ...
    {
        codTipCur = new Integer(request.getParameter("tipoCurso")).intValue();
    }
    
    if(session.getAttribute("busAluTipCur") != null)
    {
        listAluTipCur = (Vector) session.getAttribute("busAluTipCur");
    }
    ...
    if(valSup > listAluTipCur.size()) valSup = listAluTipCur.size();
...
{
	document.getElementById("fraMailing").src = "../mailing/mailing.jsp?tipoMail=2";
	$('#mailing' ).dialog( 'open' );
...
<table style="width:100%"> 
   ...
   <tr>
     ...
     <%
          String auxNomCur = "";
          if (listAluTipCur != null)
          {
              for (int ind = valInf; ind < valSup; ind++)
              {
                  curAluVO = (CursosAluVO) listAluTipCur.elementAt(ind);
                  aluVO    = AlumnosGestion.devolverDatosAlumno(curAluVO.getIdAlu());
                  curVO    = CursosGestion.devolverDatosCurso(curAluVO.getIdCur());
