...
if(session.getAttribute("nuevoTipTrastErr") != null)
{
    tipTrasVO = (TipTrastVO) session.getAttribute("nuevoTipTrastErr");
    session.removeAttribute("nuevoTipTrastErr");
}
...
{		   
	...
	{
		frmAltaTipTrast.action = "../AltaTipoTrastornoServlet";
		frmAltaTipTrast.submit();
	}
	else
	{
...
<%if(errCodAlta == -1){%>
<script>
    alert("Se produjo un error en el alta del tip de trastorno");
</script>
<%}%>
<%if(errCodAlta == -2){%>
<script>
...
<form id="frmAltaTipTrast" name="frmAltaTipTrast" method="post" action="">
<table style="width:90%" border="0">
  <tr>
    <td colspan="2" class="thDef">A&ntilde;adir Tipo Trastorno</td>
  </tr>
  <tr class="tdDef">
    <td style="width:8%" class="tdDef">Nombre</td>
    <td class="tdDef">
      <span id="txtValNombre">
        <input name="txtNombre" type="text" id="txtNombre" value="<%=tipTrasVO.getDescrip()%>" size="75" maxlength="100" />
