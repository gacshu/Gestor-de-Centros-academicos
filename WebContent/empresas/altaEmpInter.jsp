<%@page import="es.jahernandez.gestion.ListaCodPostGestion"%>
...
Vector     datProv        = ListaCodPostGestion.devolverDatProv(); //Devuelve los datos de carga de combo de Provincias
Vector     datActi        = ActividadesGestion.devolverDatActividades(); //Devuelve los datos de carga de combo de actividades
...
String     valDefCP       = "";
String     valDefCPCom    = "";
String     urlDest        = "../AltaNuevaEmpresaServlet";
String     urlProc        = "";

if(request.getParameter("urlProc") != null)
{
    urlProc = request.getParameter("urlProc");
}
...
if(session.getAttribute("nuevaEmpresaErr") != null)
{
    empVO = (EmpresasVO) session.getAttribute("nuevaEmpresaErr");
    session.removeAttribute("nuevaEmpresaErr");
}
...
else
{
    ...
    urlDest = "../AltaNuevaEmpresaServlet?urlProc=" + urlProc;
...
function validarForm()
{
	...
	validarCampos = validarCampos & 
				   sprytextfield01.validate() & sprytextfield02.validate() &  sprytextfield04.validate() &
	...
	if(validarCampos)
	{	
		document.getElementById("frmAltaEmpresa").action = "<%=urlDest%>";
...
{
    frmAltaEmpresa.txtNomCom.value    = frmAltaEmpresa.txtRazonSocial.value;
    frmAltaEmpresa.txtDirCom.value    = frmAltaEmpresa.txtDirEmp.value;
    frmAltaEmpresa.txtLocCom.value    = frmAltaEmpresa.txtLocEmp.value;
    frmAltaEmpresa.txtCodPosCom.value = frmAltaEmpresa.txtCodPosEmp.value;
    frmAltaEmpresa.selProCom.value    = frmAltaEmpresa.selProEmp.value;  
}
...
<%}%>
<form action="../AltaNuevaEmpresaServlet?urlProc=2" method="post" name="frmAltaEmpresa" target="_self" id="frmAltaEmpresa">
    ...
    <div>
    	<table style="width:72%" class="tablaApartadosEmpresa" >
    		...
    		<tr class="tdDef">  
                <td>Responsable</td>
                <td colspan="2">
                    <span id="txtValResCom">
                        <input name="txtRepCom" type="text" id="txtRepCom" value="<%=empVO.getResEmp2()%>" size="90" maxlength="100" />
                    </span>
                </td>
                <td colspan="3">
               	<input type="checkbox" name="chkDatAct" id="chkDatAct" value="true" <%if(empVO.isDatAct()){%>checked="checked"<%}%> />Datos Actualizados						                 </td>
            </tr>
    		<tr class="tdDef">
        		<td colspan="6" align="right">
        		  <input name="btnVolcarDatosFiscales" type="button" class="cellBtnSub" id="btnVolcarDatosFiscales" value="Volcar Datos  Fiscales" onclick="volcarDatosComerciales()";/>      		  </td>
       		</tr>
		</table>
    </div>
        		...
        		<td style="width:264">Fecha Convenio
                	<span id="txtValFecNac">
                		<span id="txtValFecCon">
          					<label>
            					<input class="claseForFecha" name="txtFecCon" type="text" id="txtFecCon" size="15" maxlength="10" <%if(empVO.getFecCon().after(new GregorianCalendar(1900,0,1).getTime())){%>value="<%=new SimpleDateFormat("dd/MM/yyyy").format(empVO.getFecCon())%>"<%}%>/>
          						<span class="textfieldInvalidFormatMsg">*</span>
                    		</label>
						</span>
                    		<span class="textfieldRequiredMsg">*</span>
    			...
    			<td style="width:74%">
                	<table style="width:72%" class="tablaApartadosEmpresa">
  						...
  						<tr>
    						<td>&nbsp;</td>
    						<td>
                            	<input type="checkbox" name="chkRec" id="chkRec" value="true" <%if(empVO.isRecArco()){%>checked="checked"<%}%>/>Rectificación
                            </td>
    						<td>
                            	<span id="txtValFecRec">
   									<label>
      									<input class="claseForFecha" name="txtFecRec" type="text" id="txtFecRec" size="15" maxlength="10" <%if(empVO.getFecRecArc().after(new GregorianCalendar(1900,0,1).getTime())){%>value="<%=new SimpleDateFormat("dd/MM/yyyy").format(empVO.getFecRecArc())%>"<%}%>/>
      									<span class="textfieldInvalidFormatMsg">*</span>
                                    </label>
    							</span>
                            </td>
    						<td><input type="checkbox" name="chkOposic" id="chkOposic" value="true" <%if(empVO.isOpoArco()){%>checked="checked"<%}%>/>Oposición
                            </td>
    						<td>
                            	<span id="txtValFecOpo">
    							<label>
      								<input class="claseForFecha" name="txtFecOpo" type="text" id="txtFecOpo" size="15" maxlength="10" <%if(empVO.getFecOpoArc().after(new GregorianCalendar(1900,0,1).getTime())){%>value="<%=new SimpleDateFormat("dd/MM/yyyy").format(empVO.getFecOpoArc())%>"<%}%>/>
      								<span class="textfieldInvalidFormatMsg">*</span>
                                </label>
   								</span>
                            </td>
