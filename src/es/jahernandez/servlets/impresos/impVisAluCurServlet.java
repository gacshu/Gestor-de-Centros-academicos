 ...
 * @author Alberto
 */
public class impVisAluCurServlet extends HttpServlet {
    ...
    throws ServletException, IOException
     {
        ...
        if(sesion.getAttribute("logControl") != null && sesion.getAttribute("usuario") != null)
        {
            ...
            log.info((conUsoVO.getUsuario() + "               " ).substring(0,10) + "Imprimir alumnos curso" );
        }
        ...
        alumnosBusqueda   = (Vector) sesion.getAttribute("busAluCur");
        curVO             = CursosGestion.devolverDatosCurso(((CursosAluVO)alumnosBusqueda.elementAt(0)).getIdCur());
        nivVO             = NivelesGestion.devolverDatosNivel(((CursosAluVO ) alumnosBusqueda.elementAt(0)).getIdNiv());
        ...
        try
        {
            ...
            document.add(new Paragraph("Curso: " + curVO.getNomCur() ,new Font(BaseFont.createFont(sc.getRealPath("/" + "fonts" + "/" + "cour.ttf"), BaseFont.IDENTITY_H, BaseFont.EMBEDDED))));
            ...
            for (int ind = 0; ind < alumnosBusqueda.size(); ind++)
            {
                ...
                if (! nivVO.getNomNiv().equals(auxNomNiv))
                {
                    ...
                    Paragraph parNiv = new Paragraph(nivVO.getNomNiv().trim(), new Font(BaseFont.createFont(sc.getRealPath("/" + "fonts" + "/" + "cour.ttf"), BaseFont.IDENTITY_H, BaseFont.EMBEDDED)));
