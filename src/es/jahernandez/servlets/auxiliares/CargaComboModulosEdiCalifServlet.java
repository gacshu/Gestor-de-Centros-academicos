...
public class CargaComboModulosEdiCalifServlet extends HttpServlet
{
            ...
            throws ServletException, IOException
    {
        ...
        try
        {
                ...
                for (int ind = 0; ind<vecModEdi.size(); ind ++)
                {
                    ...
                    {
                            if(valSel.trim().equals(modEdiVO.getIdMod()))
                            {
                                out.printf("<option value='%1s' selected='selected'>%2s</option>", modEdiVO.getIdMod(), ModulosGestion.devolverDatosModulo(modEdiVO.getIdMod()).getNombre());
                            }
                            else
                            {
                                out.printf("<option value='%1s'>%2s</option>", modEdiVO.getIdMod(), ModulosGestion.devolverDatosModulo(modEdiVO.getIdMod()).getNombre());
    ...
    public String getServletInfo() {
        return "Carga combo modulos edicion calificaciones servlet";
    }// </editor-fold>
}
