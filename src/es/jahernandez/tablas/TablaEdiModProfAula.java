/*
 * To change this template, choose Tools | Templates
 * and open the template in the editor.
 */
package es.jahernandez.tablas;

/**
 *
 * @author Alberto
 */
public class TablaEdiModProfAula
{
    public static final String TABLA        = "tbedimodproaul";
    public static final String CODEDI       = "IdEdi";
    public static final String CODMOD       = "IdMod";
    public static final String CODPROF      = "IdProf";
    public static final String CODAULA      = "IdAul";
    public static final String FECHAINICIO  = "fecInic";
    public static final String FECHAFIN     = "fecFin";
    public static final String HORAINICIO   = "horIni";
    public static final String HORAFIN      = "horFin";
    public static final String CLASELUNES   = "HayLun";
    public static final String CLASEMARTES  = "HayMar";
    public static final String CLASEMIERC   = "HayMie";
    public static final String CLASEJUEVES  = "HayJue";
    public static final String CLASEVIERNES = "HayVie";
    public static final String CLASESABADO  = "HaySab";
}
